...
class TestFunctions(TestCase):
    ...
    def test_get_ylims_frac_25(self):
        y_lims = get_ylims(self.train_data, self.val_data)
        target = (0.5, 3.5)
        self.assertSequenceEqual(y_lims, target)

    def test_get_ylims_frac_0(self):
        y_lims = get_ylims(self.train_data, self.val_data, fraction=0)
        target = (1, 3)
        self.assertSequenceEqual(y_lims, target)

    def test_get_ylims_no_val(self):
        y_lims = get_ylims(self.train_data, None)
        target = (0.75, 2.25)
        self.assertSequenceEqual(y_lims, target)

    def test_get_ylims_no_train(self):
        y_lims = get_ylims(None, self.val_data)
        target = (1.75, 3.25)
        self.assertSequenceEqual(y_lims, target)
