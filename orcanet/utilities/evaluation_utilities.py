...
def get_nn_predictions_and_mc_info(cfg, model, xs_mean, samples=None):
    """
    ...
    for f_number, f in enumerate(test_files):
        ...
        for s in range(steps):
            ...
            datagroups = make_datagroups(mc_info, y_true, y_pred)

            if arr_nn_pred is None:
                arr_nn_pred = np.zeros((cum_number_of_steps[-1] * batchsize, arr_nn_pred_temp.shape[1:2][0]), dtype=np.float32)
...
def make_datagroups(mc_info, y_true, y_pred):
    """
    Get the datagroups names and contents.

    Every output layer will get one datagroup each for both the label and the prediction.
    E.g. if your model has an output layer called "energy", the datagroups
    ...
    -------
    datagroups : dict
        Keys are the name of the datagroups, values the content.

    """
    datagroups = dict()
    datagroups["mc_info"] = mc_info
    for out_layer_name in y_true:
        datagroups["true_" + out_layer_name] = y_true[out_layer_name]
    for out_layer_name in y_pred:
        datagroups["reco_" + out_layer_name] = y_pred[out_layer_name]
    return datagroups
