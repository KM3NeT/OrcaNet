...
def use_node_local_ssd_for_input(train_files, test_files, multiple_inputs=False):
    ...
    if multiple_inputs is True:
        ...
        # copy xs_mean arrays to local ssd
        for i, file_input_train in enumerate(train_files[0][0]):
            files_npy = glob.iglob(os.path.join(os.path.dirname(file_input_train), '*.npy'))
            for f in files_npy:
                if os.path.isfile(f):
        ...
        files_npy = glob.iglob(os.path.join(os.path.dirname(train_files[0][0][0]), '*.npy'))
        for f in files_npy:
            if os.path.isfile(f):
                shutil.copy2(f, local_scratch_path)
